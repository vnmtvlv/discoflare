...
21. Gadgets are declarative, versioned internal applications, not generated code sandboxes. A published Gadget can compose multiple Database Views through explicit Bindings. Each Binding freezes semantic Database, View, Field, and operation identifiers; runtime reads and mutations re-authorize the current member against both their Gadget Grant and the Gadget's Role access. Jev may select from prepared sources and presentation candidates while authoring a draft, but no model runs when a published Gadget opens or invokes an action.
