...
18. The first guided installation in an account is marked Primary. Manual management discards installer OAuth after each operation. Managed operation asks the operator to create one account-owned Instance Admin Token from Discoflare's pre-filled Cloudflare template and paste it once; the installer verifies it, stores it only as a Worker secret, and does not retain it. The same token performs owner-requested self-updates and RealtimeKit control and may maintain email infrastructure. It is never exposed to browser code, D1, MCP, Members, Agents, or Agent Computers. Because Cloudflare permissions are account- and zone-scoped, a dedicated Cloudflare account is the effective isolation boundary. The Primary Worker owns its zone's mail route; another installation on the same zone must leave workspace mail disabled.
