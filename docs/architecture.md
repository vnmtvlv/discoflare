...
18. One Cloudflare account has one `discoflare-admin` control-plane Worker. Its broad credential is one account-owned token stored only there; every guided workspace receives a service binding and narrow per-Installation capability. Managed Setup creates the token with temporary OAuth, while Private Setup leaves it to the operator. Admin login uses Cloudflare identity plus a stateless encrypted cookie and requires neither Access nor D1. Admin enables RealtimeKit by default and assigns the first eligible domain-backed Installation as Primary for the zone's mail route. Discoflare.com is only the OAuth callback broker and is never in the workspace or Huddle runtime path. Because Cloudflare permissions are account- and zone-scoped, a dedicated Cloudflare account is the effective isolation boundary.
