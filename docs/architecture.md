...
18. One Cloudflare account has one `discoflare-admin` control-plane Worker. The Account Admin Token is stored only there; every guided workspace receives a service binding and narrow per-Installation capability, never the broad credential. Admin enables RealtimeKit by default and assigns the first eligible domain-backed Installation as Primary for the zone's mail route. Because Cloudflare permissions are account- and zone-scoped, a dedicated Cloudflare account is the effective isolation boundary.
