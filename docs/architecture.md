...
18. The first guided installation in an account is marked Primary. New installations start in Manual management and discard installer OAuth after each operation. The Owner may create one account-owned Instance Admin Token from the template in **Workspace Settings → Cloudflare** and paste it on the installed origin; the workspace verifies it, stores it only as its Worker secret, and enables RealtimeKit plus email when an eligible zone was explicitly selected during installation. The token never passes through `discoflare.com`, browser clients after submission, D1, MCP, Members, Agents, or Agent Computers. Because Cloudflare permissions are account- and zone-scoped, a dedicated Cloudflare account is the effective isolation boundary. The Primary Worker owns its zone's mail route; another installation on the same zone cannot claim it.
