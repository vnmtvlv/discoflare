...
Manual and Managed describe management authority, not how the installation was originally deployed. The guided installer can switch between them. Managed mode verifies one account-owned token for updates, RealtimeKit, and Cloudflare email infrastructure; Manual mode removes it from the Worker and reminds the operator to revoke it in Cloudflare. Switching a Huddle-enabled installation from Managed to Manual requires a replacement manual Realtime Admin token before the old Instance Admin Token is removed.
