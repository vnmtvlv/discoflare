...
The Base Installation does not accept or persist App Domain or Email Domain choices. After the workspace is healthy on `workers.dev`, its Settings in `discoflare.com/admin` may connect one App Domain and, for the Primary Installation, multiple Email Domains. The App Domain is the canonical HTTP origin; Email Domains are independent inbound and outbound mail identities.

Connecting an Email Domain enables Cloudflare Email Routing DNS for that exact apex or subdomain, onboards the same domain for Email Sending, and adds it to the Worker bindings. Creating a Mailbox inside the workspace creates one exact literal-address Email Routing rule through a narrow Installation Control Credential; deleting the Mailbox removes that rule first. Unknown addresses never route to the workspace. A second mail-owning Installation is refused instead of creating an auxiliary gateway Worker.
