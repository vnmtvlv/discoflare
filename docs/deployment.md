...
The first Installation in a Cloudflare account becomes Primary. It is the only Installation eligible to own account- or zone-level features such as mail routing and RealtimeKit when those cabinet flows ship. Billing and disconnecting a guided Installation into a fully independent lifecycle are deferred; the manual repository deployment remains the independent path today.
...
The lifecycle operation returns a conflict instead of replacing a foreign catch-all. Removing the Primary workspace disables its owned catch-all and removes its exact Email Sending subdomain. Workspace Workers never receive the Control Plane's broad credential.
