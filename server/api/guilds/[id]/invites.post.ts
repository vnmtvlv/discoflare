...
export default defineEventHandler(async (event) => {
  ...
  const member = await requireMember(event, guildId, Permission.invite)
  ...
  await writeAudit(env, { guildId, actorId: member.user.id, action: 'invite.create', targetType: 'invite', targetId: code })
