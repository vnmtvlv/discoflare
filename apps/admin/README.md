...
The managed installer creates an account-owned token directly as this Worker's encrypted secret, then discards its temporary OAuth grant. The private installer bootstraps the same Worker and lets the operator paste an Account Admin Token directly on the Admin origin. Workspace Workers receive neither broad credential.
