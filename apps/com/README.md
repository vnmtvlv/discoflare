...
The marketing routes are prerendered. `/deploy` is Managed Setup: temporary public-client PKCE OAuth creates the account-local Admin Worker and its account-owned token, then discards the grant. `/deploy/private` preserves the temporary bootstrap that leaves credential connection to the operator.
...
Production requires the secrets from `.env.example`. The single public OAuth client ID is declared in `wrangler.jsonc` so a Workers Build cannot erase it; register its callback as `https://discoflare.com/api/cloudflare/oauth/callback`. Managed Setup uses Token Write to create the new Admin Worker's account-owned token and deletes its temporary grant. Later Admin login reuses the same client with identity and verification read scopes. Private Setup requests no persistent token; the owner connects an Account Admin Token on their own Admin origin.
