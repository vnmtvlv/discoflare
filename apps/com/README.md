...
Production requires the secrets from `.env.example`. Register the OAuth callback as `https://discoflare.com/api/cloudflare/oauth/callback`. Managed Setup uses the explicit scopes in `server/utils/cloudflare-oauth.ts`, transfers the renewable grant into the new Admin Worker, and deletes its hosted copy. Private Setup uses the separate bootstrap client and requests no persistent token, RealtimeKit permission, email permission, or workspace data; the owner connects an Account Admin Token on their own Admin origin.
