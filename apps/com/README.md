...
The marketing routes are prerendered. `/deploy` is Managed Setup: it transfers a renewable public-client OAuth grant into the account-local Discoflare Admin Worker, then discards the hosted copy. `/deploy/private` preserves the temporary bootstrap that leaves credential connection to the operator.
...
Production requires the secrets from `.env.example`. The public Managed Setup OAuth client ID is declared in `wrangler.jsonc` so a Workers Build cannot erase it; register its callback as `https://discoflare.com/api/cloudflare/oauth/callback`. Managed Setup uses the explicit scopes in `server/utils/cloudflare-oauth.ts`, transfers the renewable grant into the new Admin Worker, and deletes its hosted copy. Private Setup uses the separate bootstrap client and requests no persistent token, RealtimeKit permission, email permission, or workspace data; the owner connects an Account Admin Token on their own Admin origin.
