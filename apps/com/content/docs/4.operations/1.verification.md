...
| `realtimekit` | Effective Live configuration is complete. |

Inspect the individual `bindings` fields required by the capability under test. A healthy D1 binding does not prove R2, KV, Durable Objects, Workflows, Workers AI, Browser Run, or Containers are ready.
...
- **Live:** run **Test connection**, then start and end a live session from two browsers.
...
- **Cloudflare Agents:** run a Task and confirm its durable result. Immediately after deployment, allow time for the Agent Computer image to provision.
