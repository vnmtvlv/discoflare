...
4. After the base workspace is healthy, open **Workspace Settings → Cloudflare → Domains** to connect its one App Domain and any Email Domains. Mailbox addresses remain workspace objects; each address gets an exact Cloudflare routing rule to its owning workspace through the same fixed per-Installation control channel.
